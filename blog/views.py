...
def serialize_post_optimized(post):
    return {
        ...
        'published_at': post.published_at,
        'slug': post.slug,
        'tags': [serialize_tag_optimized(tag) for tag in post.tags.all().annotate(posts_count=Count('posts'))],
        'first_tag_title': post.tags.all()[0].title,
    }
...
def index(request):
    ...
    fresh_posts = Post.objects.annotate(comments_count=Count('comments', distinct=True)).order_by(
        'published_at').prefetch_related('author', 'tags')
